...
<div class="space-y-5">
	<div class="flex items-center justify-between gap-4">
		...
		<div class="flex gap-2">
			<button
				...
				disabled={syncing}
				class="inline-flex h-10 items-center gap-2 rounded-lg bg-zinc-200 px-3 sm:px-4 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-300 active:bg-zinc-400 disabled:opacity-50 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"
			>
				...
				<span class="hidden sm:inline">{syncing ? 'Syncing...' : 'Refresh All'}</span>
			</button>
			<a
				href="/podcast/add"
				class="inline-flex h-10 items-center gap-2 rounded-lg bg-emerald-600 px-3 sm:px-4 text-sm font-medium text-white transition-colors hover:bg-emerald-700 active:bg-emerald-800"
			>
				<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
				</svg>
				<span class="hidden sm:inline">Add Podcast</span>
			</a>
		</div>
	</div>
	...
	{:else if podcasts.length === 0}
		<div class="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-zinc-300 py-20 dark:border-zinc-700">
			...
			<a
				href="/podcast/add"
				class="inline-flex h-10 items-center gap-2 rounded-lg bg-emerald-600 px-4 text-sm font-medium text-white transition-colors hover:bg-emerald-700"
			>
				<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
				</svg>
		...
		<div class="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
			{#each podcasts as podcast (podcast.id)}
				<a
					...
					class="group overflow-hidden rounded-xl border border-zinc-200 bg-white transition-all hover:shadow-lg hover:ring-2 hover:ring-emerald-500/30 active:scale-[0.98] dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zinc-700"
				>
