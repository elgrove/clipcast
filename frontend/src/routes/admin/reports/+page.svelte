...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		...
		<button
			...
			class="inline-flex items-center gap-2 rounded-lg bg-zinc-200 px-3 py-2 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-300 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"
		>
				...
				<tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
					{#each reports as report (report.id)}
						<tr class="bg-white transition-colors hover:bg-zinc-50 dark:bg-zinc-950 dark:hover:bg-zinc-900/50">
									...
									<div class="text-xs">
										{#if report.transcription_input_tokens !== null}
											<div class="text-zinc-500 dark:text-zinc-400">
												T: {formatTokens(report.transcription_input_tokens)}↓ {formatTokens(report.transcription_output_tokens)}↑
											</div>
										{/if}
										{#if report.analysis_input_tokens !== null}
											<div class="text-zinc-500 dark:text-zinc-400">
												A: {formatTokens(report.analysis_input_tokens)}↓ {formatTokens(report.analysis_output_tokens)}↑
											</div>
										{/if}
									</div>
								{:else}
							...
							<td class="px-4 py-3">
								{#if (report.transcription_cost && report.transcription_cost > 0) || (report.analysis_cost && report.analysis_cost > 0)}
									<div class="text-xs">
										<div class="text-zinc-900 dark:text-zinc-100">
											{formatCost((report.transcription_cost || 0) + (report.analysis_cost || 0))}
										</div>
									</div>
								{:else}
									<span class="text-zinc-400 dark:text-zinc-600">—</span>
